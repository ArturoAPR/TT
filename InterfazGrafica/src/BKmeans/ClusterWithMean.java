...
public class ClusterWithMean extends Cluster {
	private DoubleArrayName mean;  // the mean of the vectors in this cluster
	
	DoubleArrayName sum; // the sum of all vectors in this clusters 
	...
	public ClusterWithMean(int vectorsSize){
		super();
		sum = new DoubleArrayName(new double[vectorsSize]);
	}
	 ...
	 */
	public void setMean(DoubleArrayName mean){
		this.mean = mean;
	}
	 ...
	 */
	public void addVector(DoubleArrayName vector) {
	 ...
	 */
	public DoubleArrayName getmean() {
		return mean;
	}
	 ...
	 */
	public void remove(DoubleArrayName vector) {
		super.remove(vector);
		// remove from sum
		for(int i=0; i < vector.data.length; i++){
			sum.data[i] -= vector.data[i];
