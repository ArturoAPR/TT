...
public class AlgoKMeans {
	...
	protected List<ClusterWithMeanID> clusters = null;
	 ...
	 */
	public List<ClusterWithMeanID> runAlgorithm(String inputFile, int k, DistanceFunction distanceFunction) throws NumberFormatException, IOException {
		...
		List<DoubleArrayName> vectors = new ArrayList<DoubleArrayName>();
		...
		while (((line = reader.readLine()) != null)) {
                  //  System.out.println("FileNAme:"+line.substring(line.indexOf('[')+1, line.indexOf(']')));
                  //  System.out.println("line"+line.substring(line.indexOf(']')+1,line.length() ));
                    fileName=line.substring(line.indexOf('[')+1, line.indexOf(']'));
			...
			// add the vector to the list of vectors
                        DoubleArrayName dan = new DoubleArrayName(vector);
                        dan.setNombreArchivo(fileName);
			vectors.add(dan);
		...
		if(k == 1) {
			...
			ClusterWithMeanID cluster = new ClusterWithMeanID(vectorsSize);
			for(DoubleArrayName vector : vectors) {
				cluster.addVector(vector);
			}
			cluster.setMean(new DoubleArrayName(new double[vectorsSize]));
		...
		if (vectors.size() == 1) {
			// Create a single cluster and return it 
			clusters = new ArrayList<ClusterWithMeanID>();
			DoubleArrayName vector = vectors.get(0);
			ClusterWithMeanID cluster = new ClusterWithMeanID(vectorsSize);
			cluster.addVector(vector);
			cluster.recomputeClusterMean();
			cluster.setMean(new DoubleArrayName(new double[vectorsSize]));
	...
	void applyAlgorithm(int k, DistanceFunction distanceFunction,
		...
		//clusters = applyKMeans(k, distanceFunction, vectors, minValue, maxValue, vectorsSize);
                ...
                for(int i = 0; i < 100; i++) {
                //    System.out.println("Kameans "+i);
				List<ClusterWithMeanID> newClusters = applyKMeans(k, distanceFunction, vectors, minValue, maxValue, vectorsSize);
                ...
                clusters=bestClustersUntilNow;
                
                
	}
        public void aplicarKmeans(int k, DistanceFunction distanceFunction,
			List<DoubleArrayName> vectors, double minValue, double maxValue,
			int vectorsSize){
                        System.out.println("aplicarKmenas");
                System.out.println("Size: "+vectorsSize);
                List<ClusterWithMeanID> newClusters = applyKMeans(k, distanceFunction, vectors, minValue, maxValue, vectorsSize);                                
                
                                                    
                
                    for(int i=0;i<newClusters.size();i++){                                                                                              
                        for(int j=0;j<newClusters.get(i).getVectors().size();j++){
                            double value = newClusters.get(i).getVectors().get(j).get(0);
                            if(value < minValue){
					minValue = value;
				}
				if(value > maxValue){
					maxValue = value;
                        ...
                        newClusters.get(i).setId(vectorsSize+"_"+i);
                        System.out.println(vectorsSize+"_"+i);
                        aplicarKmeans(k,distanceFunction,newClusters.get(i).getVectors(),minValue,maxValue,newClusters.get(i).getVectors().size());
                    }
	 ...
	 * @param vectorsSize  the vector size
	 */
	List<ClusterWithMeanID> applyKMeans(int k, DistanceFunction distanceFunction,
			List<DoubleArrayName> vectors, double minValue, double maxValue,
			int vectorsSize) {
		List<ClusterWithMeanID> newClusters = new ArrayList<ClusterWithMeanID>();
		...
		// center)
		for(int i=0; i< k; i++){
			DoubleArrayName meanVector = generateRandomVector(minValue, maxValue, vectorsSize);
			ClusterWithMeanID cluster = new ClusterWithMeanID(vectorsSize);
		...
		while(true) {
			...
			// (2.1) Assign each point to the nearest cluster center.
                        ClusterWithMeanID nearestCluster = null;
			// / for each vector
			for (DoubleArrayName vector : vectors) {
				// find the nearest cluster and the cluster containing the item
				nearestCluster = null;
				ClusterWithMeanID containingCluster = null;
				double distanceToNearestCluster = Double.MAX_VALUE;

				// for each cluster
				for (ClusterWithMeanID cluster : newClusters) {
			...
			if(!changed){     // exit condition for main loop
                             System.out.println("CENTRO: "+nearestCluster.getmean());
				break;
			}
	 ...
	 */
	DoubleArrayName generateRandomVector(double minValue, double maxValue,
			int vectorsSize) {
		// create a new vector
		...
		// return the vector
		return new DoubleArrayName(vector);
	}
